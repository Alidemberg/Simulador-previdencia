  ...
  return (
    <div>
      ...
      {idadeEquilibrio !== null && (
        <div style={{
          background: "#0a0a0a", border: `1px solid ${idadeEquilibrio > 100 ? RED : GREEN}44`,
          borderRadius: 8, padding: 14, marginBottom: 16, textAlign: "center"
        }}>
          <div style={{ color: MUTED, fontSize: 10, letterSpacing: 2, textTransform: "uppercase", fontFamily: "'DM Mono', monospace", marginBottom: 6 }}>
            Ponto de Equilíbrio Financeiro
          </div>
          {idadeEquilibrio > 0 && idadeEquilibrio < 120 ? (
            <>
              <div style={{ color: idadeEquilibrio > 80 ? RED : GREEN, fontSize: 28, fontFamily: "'DM Mono', monospace", fontWeight: 700 }}>
                {Math.round(idadeEquilibrio)} anos
              </div>
              <div style={{ color: MUTED, fontSize: 11, fontFamily: "'DM Mono', monospace", marginTop: 6 }}>
                {idadeEquilibrio <= 80
                  ? `✅ A especial supera o 100% antes dos 80 anos — vale antecipar.`
                  : `⚠️ Só compensa a especial se você viver além dos ${Math.round(idadeEquilibrio)} anos.`}
              </div>
            </>
          ) : (
            <div style={{ color: MUTED, fontSize: 13, fontFamily: "'DM Mono', monospace" }}>
              {valorEsp >= valorCem ? "Especial já é 100% — ponto de equilíbrio imediato." : "Não há ponto de equilíbrio prático."}
            </div>
          )}
        </div>
      )}
